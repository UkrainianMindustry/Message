    ...
    void register() {
        ...
        adminHandler.<Message>register("warn", "<@user> [причина...]", "Видати попередження користувачеві.", (args, msg) -> {
            ...
            try {
                ...
                list.add(System.currentTimeMillis() + ":::" + msg.getAuthor().getName() + (args.length > 1 ? ":::" + violatedRules : ""));
...
//                return true;
    ...
    String[] getRulesArray() {
        // **1.** Text
        Pattern pattern = Pattern.compile("\\*\\*(\\d+)\\.\\*\\*\\s+(.*?)\\n");

        Matcher matcher = pattern.matcher(Info.rules.text);
